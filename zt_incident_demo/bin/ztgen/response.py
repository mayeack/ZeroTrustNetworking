...
def next_request_id(splunkd, now):
    """One number above the highest request ID of the day, wherever it was issued: local requests live in the KV
    store, the SOAR playbook's only in the audit trail, the 16 routine actions use 0001-0016."""
    day = time.strftime("%Y%m%d", time.gmtime(now))
    seqs = [16]
    for r in splunkd.kv_query(REQUESTS, {"request_id": {"$regex": "^ZTR-%s-" % day}}):
        try:
            seqs.append(int(str(r.get("request_id", "")).rsplit("-", 1)[1]))
        except (IndexError, ValueError):
            pass
    try:
        rows = splunkd.search('search index=zero_trust sourcetype=zt:enforcement:audit request_id="ZTR-%s-*" | rex field=request_id "-(?<seq>\\d{4})$" | stats max(seq) as last_seq' % day,
                              earliest="-26h", latest="now", timeout=60)
        if rows and rows[0].get("last_seq"):
            seqs.append(int(float(rows[0]["last_seq"])))
    except RestError:
        pass
    return "ZTR-%s-%04d" % (day, max(seqs) + 1)
...
def create_request(splunkd, hec, brief, now=None):
    ...
    job_id = str(brief.get("job_id") or "").strip()
    if not job_id.isdigit():
        # the story's runner always runs job 88213; any other pod without a CI job is quarantined under its own name
        job_id = str(canon.CI_JOB_ID) if pod == canon.RUNNER_POD else ""
    ...
    req = {"_key": brief.get("_key") or finding_id, "request_id": next_request_id(splunkd, now), "finding_id": finding_id, "investigation_id": brief.get("investigation_id", ""),
           ...
           "action": ("CNP %s" % built["policy_name"]) if point == "kernel" else (brief.get("action") or "CNP %s" % built["policy_name"]), "agent_action": brief.get("action", ""), "policy_name": built["policy_name"], "policy_yaml": built["policy_yaml"],
...
def approval_message(req):
    job = str(req.get("job_id") or "")
    return cnp.approval_message(req.get("investigation_id") or req["finding_id"], req["workload"], req.get("dest_workload", ""), req.get("data_class", ""), req.get("disposition", ""),
                                req.get("confidence", ""), req.get("what_happened", ""), req["action"], req["enforcement_point"], req.get("blast_radius", ""), req["policy_yaml"],
                                req["pod"], job if job.isdigit() else req["pod"], req["policy_name"], request_id=req.get("request_id", ""))
