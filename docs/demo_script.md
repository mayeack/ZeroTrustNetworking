...
2. Switch to Splunk Secure Networking Essentials (beta), Secure Data Center Operations, 24h. The view opens on Fabric Operations: select the Runtime Security tab. Runtime Detections 2, Active Isovalent Nodes 87, Observed Namespaces 129, Process Executions Observed about 12,000, Telemetry Age 0. Say: "Every node and namespace Tetragon sees, in a product view nobody had to build. And the two processes Tetragon policy killed today, block-reverse-shell and block-unapproved-binaries."
3. Runtime Investigation tab: Top workloads by process execution volume, `build-farm / ci-runner` first; the detection policy table below lists the two policy alerts. Say: "Notice the busiest workload: the build farm's CI runner. Hold that thought." Then fire.
4. If asked why the incident is not in that list: Runtime Detections shows Tetragon policy alerts (processes killed by a tracing policy); the story's protected-path detections run in Enterprise Security. If asked about the fabric: Secure Infrastructure › Infrastructure Posture › Data Center Signals shows the day's Nexus Dashboard anomalies (5) and advisories, with one critical advisory; Fabric Operations › Critical Security Advisories shows the same advisory. Stay off the other Fabric Operations and Data Center Security cards: they read ACI, APIC and Secure Firewall data, and this fabric is NX-OS. Say beta; give no availability date.
...
4. Open Zero Trust Incident › Zero Trust Fabric Posture. Read the KPIs: Protected-Path Coverage 87.9%, "up from 86.4%"; Unprotected Paths 8, "down from 9"; Workload Identities 1,284; Enforcement Actions (24h) 17 (kernel 11 · DPU 2 · switch 4). These are the numbers when no other incident ran in the last 24 hours; after rehearsals each run moves the same way from a lower base (for example 85.3% during, 86.8% after, unprotected paths 10 to 9). Say: "During the incident the fabric had one more unprotected path; now it has one more enforced path and one more kernel action."
...
6. Point at this incident's row in the Enforcement Audit Trail, normally the top row (a routine enforcement can land above it): the finding, `CNP zt-quarantine-ci-runner-88213`, `kernel`, `j.chen (SOC tier 2)`, `verified` in green. Say: "Who approved what, when, and that it worked, in the same index as the flows."
